...
- [`ui-spec-auditor`](.agents/skills/ui-spec-auditor/SKILL.md): UI Compliance Auditor — cross-checks designed or implemented UI screens against Task specs, docs_scan reference images, RBAC matrix, and business rules (No-SKU, field order, state-driven logic, mobile UX). Produces a structured Audit Report (score /50, PASS/WARN/FAIL per dimension). Operates as the independent counterpart of `ui-ux-flow-designer` to form a closed **Design → Audit → Fix** loop. Gate rule: screen must score ≥ 40/50 with 0 FAIL items before implementation handoff.
...
- [`pencil-ui-designer`](file:///d:/Projects/logistics-website/.agents/skills/pencil-ui-designer/SKILL.md): Dedicated agent & skill for vector UI/UX design, screen layout creation & editing, wireframing, and web UI conversion directly via Pencil MCP. Operates strictly inside `pencil-workspace/pens/` with automatic file save & disk synchronization on every edit.
...
- `pencil-ui-designer`: Specialized subagent for authoring vector UI/UX canvas screens, editing layouts, and converting web UIs using Pencil MCP tools strictly inside `pencil-workspace/`.
